		...
		use global, only: cld_spi,modcldphase 
		...
		real :: vscale
		integer (kind=2),allocatable :: cld_spi_1(:,:,:)
...
!		print *,fname,shape(modcldphase),modcldphase(1:2,1:2)

		sds_name='Cloud_Mask_SPI'
        sid=sfn2index(fid,'Cloud_Mask_SPI')
        sd =sfselect(fid,sid)
		status=sfginfo(sd,sds_name,rank,dim_sizes,data_type,n_attrs)
		start(1:3)=0
		edges(1)=dim_sizes(1)
