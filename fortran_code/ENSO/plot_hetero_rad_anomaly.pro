...
pro plot_hetero_rad_anomaly
  ...
  for mi=0,3 do begin
    ...
    ; to plot
     posx1=[0.06,0.22,0.38,0.54,0.70,0.86]
     posx2=[0.18,0.34,0.50,0.66,0.82,0.98]
	 ...
	 for mi=0,3 do begin
	    ...
	    if mi eq 0 then begin
 	     im=image(data1,lon,lat,rgb_table=72,min_value=rmin,max_value=rmax,dim=[850,450],position=pos1)
 	     t21=text(pos1[0]+0.03,pos1[3]-0.02,subtitle1[0],font_size=fontsz)
 	     t22=text(pos2[0]+0.03,pos2[3]-0.02,subtitle1[1],font_size=fontsz)
        ...
        t1=text(pos1[0]+0.01,pos1[3]-0.05,subtitle[0,mi],font_size=fontsz)
		im.scale,1,1.2
      ...
      mc=mapcontinents(/continents,transparency=0,fill_color='white')
      mc['Longitudes'].label_angle=0
      t2=text(pos2[0]+0.01,pos2[3]-0.05,subtitle[1,mi],font_size=fontsz)
	  im1.scale,1,1.2
      ...
      mc=mapcontinents(/continents,transparency=0,fill_color='white')
      mc['Longitudes'].label_angle=0
      t3=text(pos3[0]+0.01,pos3[3]-0.05,subtitle[2,mi],font_size=fontsz)
	  im2.scale,1,1.2
     ...
     mc=mapcontinents(/continents,transparency=0,fill_color='white')
     mc['Longitudes'].label_angle=0
     t4=text(pos4[0]+0.01,pos4[3]-0.05,subtitle[3,mi],font_size=fontsz)
	 im3.scale,1,1.2

     im4=image(data5,lon,lat,rgb_table=72,min_value=hmin,max_value=hmax,$
         position=pos5,/current)
     mp=map('Geographic',limit=maplimit,transparency=0,overplot=im4)
     grid=mp.MAPGRID
     grid.label_position=0
     grid.linestyle='dotted'
     grid.grid_longitude=20
     ...
     mc=mapcontinents(/continents,transparency=0,fill_color='white')
     mc['Longitudes'].label_angle=0
     t4=text(pos5[0]+0.01,pos5[3]-0.05,subtitle[4,mi],font_size=fontsz)
	 im4.scale,1,1.2
 
     im5=image(data6,lon,lat,rgb_table=72,min_value=hmin,max_value=hmax,$
         position=pos6,/current)
     mp=map('Geographic',limit=maplimit,transparency=0,overplot=im5)
     grid=mp.MAPGRID
     grid.label_position=0
     grid.linestyle='dotted'
     grid.grid_longitude=20
     ...
     mc=mapcontinents(/continents,transparency=0,fill_color='white')
     mc['Longitudes'].label_angle=0
     t5=text(pos6[0]+0.01,pos6[3]-0.05,subtitle[5,mi],font_size=fontsz)
	 im5.scale,1,1.2
	
	endfor ; endfor plot season  
	barpos1=[0.06,0.09,0.34,0.11]
	barpos2=[0.38,0.09,0.66,0.11]
	barpos3=[0.70,0.09,0.98,0.11]
	...
	ct1=colorbar(target=im4,title='$H_{\sigma}$ Anomaly',font_size=fontsz,border=1,position=barpos3,orientation=0)

	im.save,'ENSO_spatial_radiation_hetero_anomaly_R05.png'
  stop
