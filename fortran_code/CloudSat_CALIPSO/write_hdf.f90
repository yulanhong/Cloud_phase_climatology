...
!**** End of variable declaration ************************************
        ...
        FORALL (i=1:NM_hgt) cf_hgt(i)=i-1
!***************** change datatype *******************************
...
!***************WRITE GEODATAFIELD************************************     
        ...
        factor=1.0
        offset=0.0
        status=sfwdata ( sds_id, start, stride, edges, time )
        status=sfsattr(sds_id,'Longname',DFNT_CHAR8,8,'UTC time')
        status=sfsattr(sds_id,'units',DFNT_CHAR8,1,'s') 
...
!        SDS_NAME="SZA"
!        sds_id = sfcreate(fid,SDS_NAME,DFNT_INT16,RANK,dim_sizes)
!        status=sfwdata ( sds_id, start, stride, edges,SZA_noon )
!        status=sfsattr(sds_id,'Longname',DFNT_CHAR8,11,'SZA at noon')
!        status=sfsattr(sds_id,'units',DFNT_CHAR8,6,'degree') 
!        status=sfsnatt(sds_id,'offset',DFNT_FLOAT32,1,offset)
!        status=sfsnatt(sds_id,'factor',DFNT_FLOAT32,1,factor)
!        datarange(1)=0
!        datarange(2)=900
!        status=sfsnatt(sds_id,'range',DFNT_INT16,2,datarange)
