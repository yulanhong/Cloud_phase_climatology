...
!****************************************************************
        ...
        subroutine read_cloudsat(file_name)
	...
	character (len=120) :: file_name,swathname,fieldname
        ...
        allocate(cbase(NCOL_S,NROW))
        allocate(ctop(NCOL_S,NROW))
        allocate(wc_top(NCOL_S,NROW))
        allocate(lon(NROW))
        allocate(lat(NROW))
        allocate(clayer(NROW))
        allocate(cphase(NCOL_S,NROW))
!        allocate(preci_flag(NCOL_S,NROW)) 
	allocate(map_hgt(NROW))
...
!	print *,fid
	...
	status = swrdfld(swid,'DEM_elevation',start,stride,dim_sizes(2:2),map_hgt)
!	print *,file_name,map_hgt(10000:10005)
!	stop
        !=======2 D============
        status = swrdfld(swid,'CloudLayerBase',start,stride,&
                        dim_sizes(1:rank),cbase)
        status = swrdfld(swid,'CloudLayerTop',start,stride,&
                        dim_sizes(1:rank),ctop)
        status = swrdfld(swid,'Water_layer_top',start,stride,&
                        dim_sizes(1:rank),wc_top)
        status = swrdfld(swid,'CloudPhase',start,stride,&
                        dim_sizes(1:rank),cphase)
!        status = swrdfld(swid,'PrecipitationFlag',start,stride,&
!                        dim_sizes(1:rank),preci_flag)
!        stop
