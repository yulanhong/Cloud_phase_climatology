   ...
   pro plot_ver_8p,oneicnum,oneminum,liqonlynum,mulicnum,liqicenum,icemixnum,obsnum,onenum,mulnum,lon,lat

   nx=n_elements(lon)
   ny=n_elements(lat)
   hgt=25-findgen(101)*0.25
   monind=[6,7,8]
   lonind=8 ;8,10 107.5,117.5,127.5
   lontext='Lon 117.5$\deg$'
   tponenum=total(onenum[*,*,*,monind],4)
   tponeicnum=total(oneicnum[*,*,*,monind],4) 
   tponeminum=total(oneminum[*,*,*,monind],4) 
   tpliqonlynum=total(liqonlynum[*,*,*,monind],4) 
   tpmulicnum=total(mulicnum[*,*,*,monind],4) 
   tpliqicenum=total(liqicenum[*,*,*,monind],4) 
   tpicemixnum=total(icemixnum[*,*,*,monind],4) 
   tpmulnum=total(mulnum[*,*,*,monind],4) 
...
;         '','','0.065','','','','','0.085']
   ...
   For i=0,7 Do Begin
	...
	xtitle=''
	ytitle='Altitude (km)'
 	verfre=reform(tponenum[lonind,*,*])/reform(tpobsnum[lonind,*,*])
	...
	xtitle='Latitude (deg)'
	ytitle='Altitude (km)'
 	verfre=reform(tpmulnum[lonind,*,*])/reform(tpobsnum[lonind,*,*])
	...
	ytitle=''
	pos=pos3
 	verfre=reform(tponeicnum[lonind,*,*])/reform(tpobsnum[lonind,*,*])
    endif     
	...
	xtitle='Latitude (deg)'
	ytitle=''
 	verfre=reform(tpmulicnum[lonind,*,*])/reform(tpobsnum[lonind,*,*])
    endif

    if i eq 4 then begin
	pos=pos5
	title='e) Liquid Cloud no Ice above '
	xtitle=''
	ytitle=''
 	verfre=reform(tpliqonlynum[lonind,*,*])/reform(tpobsnum[lonind,*,*])
    endif     
	...
	xtitle='Latitude (deg)'
	ytitle=''
 	verfre=reform(tpliqicenum[lonind,*,*])/reform(tpobsnum[lonind,*,*])
    endif
	...
	xtitle=''
	ytitle=''
 	verfre=reform(tponeminum[lonind,*,*])/reform(tpobsnum[lonind,*,*])
    endif     
	...
	ytitle=''
	pos=pos8
 	verfre=reform(tpicemixnum[lonind,*,*])/reform(tpobsnum[lonind,*,*])
    endif     
