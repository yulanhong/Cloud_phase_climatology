...
read_dardar,fname,'monthly_180_stat',lowstat
...
c=image(surf_T,lon,lat,xrange=[min(lon),max(lon)],yrange=[min(lat),max(lat)],$
     max_value=216,min_value=210,title=subtitle[mi],$
...
c2=image(surf_rh,lon,lat,xrange=[min(lon),max(lon)],yrange=[min(lat),max(lat)],$
     max_value=0.04,min_value=0.002,$
...
c.save,'cldsat_180hpa_2d_fourseason_day.png'
